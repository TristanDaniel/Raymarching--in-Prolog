...
main :-
    open("720p5withmaybegoodlight.bmp", write, Stream, [encoding(octet)]),
    write_header(Stream),
    write_pixels(Stream),
    close(Stream),
...
write_pixels(Stream) :-
    ...
    thread_create(threaded_pix_loop(-640, -357, -64, CX, CY, CZ), _, [detached(true)]),
    thread_create(threaded_pix_loop(-638, -357, -32, CX, CY, CZ), _, [detached(true)]),
    thread_create(threaded_pix_loop(-636, -357, 0, CX, CY, CZ), _, [detached(true)]),
    thread_create(threaded_pix_loop(-634, -357, 32, CX, CY, CZ), _, [detached(true)]),
    thread_create(threaded_pix_loop(-640, -354, 64, CX, CY, CZ), _, [detached(true)]),
